    ...
    def run(self, test_mode=False):
        ...
        while not terminated:
            ...
            if test_mode and self.args.render:
                ...
                self.env.render()
                if 'stag_hunt' in self.args.env:
                    print(self.env._grid.tolist())
                    self.logger.log_scalar('grid', self.env._grid.tolist())
                elif 'foraging' in self.args.env:
                    player_positions = [player.position for player in self.env.env.players]
                    player_levels = [player.level for player in self.env.env.players]
                    food_positions = [[int(x), int(y)] for (x, y) in zip(*self.env.env.field.nonzero())]
                    food_levels = [int(self.env.env.field[x, y]) for [x, y] in food_positions]
                    self.logger.log_scalar('player_positions', [[x, y] for (x, y) in player_positions])
